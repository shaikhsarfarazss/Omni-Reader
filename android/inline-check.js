...
`;
        ...
        // ===================================================
        // SAVE: Current visible page as PNG + persist to PDF mode
        // ===================================================
        async function saveEditorChanges() {
            try {
                showToast('Saving current page...');

                // 1. Determine current active page based on viewport scroll
                let currentPageNum = 1;
                let pageTopFabric = 0;
                let pageHeightFabric = 0;
                let visibleLeft, visibleTop, visibleWidth, visibleHeight;
                let dataUrl, annotDataUrl;

                const multiplier = (window.innerWidth < 1024) ? 2.0 : 3.5;

                if (pdfDoc && fabricCanvas) {
                    const vpt = fabricCanvas.viewportTransform;
                    const zoom = fabricCanvas.getZoom();

                    // The center of the visible area in canvas coordinates
                    const visibleCenterY = (-vpt[5] + fabricCanvas.height / 2) / zoom;

                    // Find which background image we are looking at
                    ...
                    if (targetBg) {
                        ...
                        // Capture the standalone page for "Save" to camera roll
                        ...
                        // 2. Capture just the transparent annotations to overlay in PDF mode
                        // Hide backgrounds and canvas background color so export is truly transparent
                        ...
                        annotDataUrl = fabricCanvas.toDataURL({
                            format: 'png',
                            multiplier: 1.0, // Screen size is enough for overlay
                            left: visibleLeft,
                            top: visibleTop,
                            width: visibleWidth,
                            height: visibleHeight
                ...
                // If not in a proper state, fallback to basic viewport capture
                if (!dataUrl) {
                    const vpt = fabricCanvas.viewportTransform;
                    const zoom = fabricCanvas.getZoom();
                ...
                // 3. Persist annotation overlay to localStorage for PDF mode cross-sync
                // ONLY store the lightweight annotation image, NOT the full canvas JSON
                // (full canvas JSON causes QuotaExceededError â€” 5MB localStorage limit)
                if (annotDataUrl) {
                    try {
                        localStorage.setItem(`annot_page_${currentPageNum}`, annotDataUrl);
                        localStorage.setItem('omnireader_editor_saved', 'true');
                    } catch (quotaErr) {
                        console.warn('Annotation too large for localStorage, using reduced quality');
                        ...
                        try { localStorage.setItem(`annot_page_${currentPageNum}`, smallAnnot); } catch (e) { }
                    }
                }

                // 4. Save file to device
                const fileName = `OmniReader_Page_${currentPageNum}_${new Date().getTime()}.png`;
                if (window.Capacitor && window.Capacitor.Plugins && window.Capacitor.Plugins.Filesystem) {
                    const Filesystem = window.Capacitor.Plugins.Filesystem;
                    const base64Data = dataUrl.split(',')[1];
                    await Filesystem.writeFile({
                        path: `Pictures/${fileName}`, // specifically requesting Gallery/Pictures
                        data: base64Data,
                        directory: 'EXTERNAL_STORAGE',
                        recursive: true
                    });
